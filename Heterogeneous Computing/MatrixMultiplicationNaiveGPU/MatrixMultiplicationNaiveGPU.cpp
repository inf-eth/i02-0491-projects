...
CMatrixMultiplicationNaiveGPU::CMatrixMultiplicationNaiveGPU(unsigned int pRows, unsigned int pCols): Rows(pRows), Cols(pCols), MatrixA_(NULL), MatrixB_(NULL), /*MatrixBt_(NULL),*/ MatrixC_(NULL), tStart(0LL), tEnd(0LL), tDelta(0LL), tPaused(true)
{
...
int CMatrixMultiplicationNaiveGPU::InitialiseCL()
{
	cl_int status = 0;
...
int CMatrixMultiplicationNaiveGPU::RunCLKernels(unsigned int Iterations)
{
	cl_int status;
	cl_uint maxDims;
	cl_event events[2];
	...
	for (unsigned int n=0; n<Iterations; n++)
	{
		...
		// Enqueue read buffer.
		SafeCall(clEnqueueReadBuffer(commandQueue, d_MatrixC_, CL_TRUE, 0, sizeof(PRECISION)*Rows*Cols, MatrixC_, 0, NULL, &events[1]), "Error: clEnqueueReadBuffer failed. (clEnqueueReadBuffer)");
...
int CMatrixMultiplicationNaiveGPU::CompleteRun(unsigned int Iterations)
{
	...
	SafeCall(RunCLKernels(Iterations), "Error: Running kernels (GPU).");
	//DisplayMatrices();
...
void CMatrixMultiplicationNaiveGPU::DisplayMatrices()
{
	cout << "Matrix A:" << endl;
	Display(MatrixA_);
	cout << "Matrix B:" << endl;
